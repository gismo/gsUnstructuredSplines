...
for idx, compute in enumerate(compute_list):
    if compute:
        for geo in geo_list:
            for deg in deg_list[idx]:
                if method_list[idx] == Method.ApproxC1:
                    path_bvp = "results/" + geo + "/bvp/" + "approxC1" + "-bvp1-p" + str(deg_approxC1) \
                               + "-r" + str(deg_approxC1-1) + "-P" + str(deg) + "-R" + str(deg-1) + "-l" + str(loop)
                    lib.create_biharmonic_bvp(path_geo=path_geo + geo, loop=loop, deg=deg, reg=deg-1, path_bvp=path_bvp,
                                              method=method_list[idx], ptilde=deg, rtilde=deg-1)
                else:
                    path_bvp = "results/" + geo + "/bvp/" + "nitsche" + "-bvp1-p" + str(deg) + "-r" + str(deg-1) + \
                               "-l" + str(loop)
                    lib.create_biharmonic_bvp(path_geo=path_geo + geo, loop=loop, deg=deg, reg=deg-1, path_bvp=path_bvp,
        ...
        for geo in geo_list:
            for deg in deg_list[idx]:
                if method_list[idx] == Method.ApproxC1:
                    path_bvp = "results/" + geo + "/bvp/" + "approxC1" + "-bvp1-p" + str(deg_approxC1) + "-r" \
                               + str(deg_approxC1-1) + "-P" + str(deg) + "-R" + str(deg-1) + "-l" + str(loop)
                else:
                    path_bvp = "results/" + geo + "/bvp/" + "nitsche" + "-bvp1-p" + str(deg) + "-r" + str(deg-1) + \
                               "-l" + str(loop)
                path_output = path_bvp.replace("bvp/", "results/")
...
for method in method_list:
    ...
    if method == Method.ApproxC1:
        ...
        for geo in geo_list:
            ...
            name_mat_list.append(geo + "-" + m_str + "-jump-p" + str(deg) + "-r" + str(deg-1) + "-l" + str(loop))
...
for idx in range(len(method_list)):
    list_tikz2, legend_list2 = create_tikz_figures(geo_mat_coll[idx], name_mat_coll[idx], deg_list[idx],
                                                   [opt_plot[idx]], shift[idx])
...
for idx, method in enumerate(method_list):
    if method == Method.ApproxC1:
        ...
        for deg in deg_list[idx]:
            legend_image.append(["empty legend"])
            legend_entry.append(
                [r'\hspace{-1.7cm}$\widetilde{p}=' + str(deg) + '$, $\widetilde{r}=' + str(deg - 1) + '$'])
        fig = lib.MyTikz()
...
for idx in range(len(method_list)):
    lib.clean_extensions(crop=legend_list[idx])
